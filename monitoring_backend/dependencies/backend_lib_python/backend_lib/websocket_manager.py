...
class Websocket_Wrapper:
    ...
    async def authenticate(self):
        ...
        try:
            ...
            await ws.send_json({"success": True, "action": "auth", "timeout": self.timeout})
            return True
...
class WebsocketManager:
    ...
    def __init__(self, debug=False, zmq=True, timeout=TIMEOUT, servicename="Unknown"):
        ...
        self.register("keepalive", lambda data, ws_wrapper: self.keepalive_callback())

        self.debug = debug
        self.zmq = zmq
        self.timeout = TIMEOUT
        if timeout < 5:
            print("Warning: timeout is too low. Setting to default value.")
        else:
            self.timeout = timeout
    ...
    # send message to all connected users and clients with same wdcode as ws_wrapper
    # data has to contain "action"
    async def send_all(self, data, ws_wrapper, success=None):
        if not self.zmq:
            print("Error! send without zmq!")
            return False
        ...
        if not "action" in data.keys():
            print("Error! No action in send data!")
            return False
        if not "success" in data.keys() and success is not None:
            data["success"] = success
        try:
            self.pending_zmq.append(ws_wrapper)
            success = await self.send_zmq(data, wdcode)
    ...
    async def keepalive_callback(self):
        return True
